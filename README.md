...
```
`#include <WiFi.h>`    
&emsp;Thư viện hỗ trợ các chức năng cho WiFi cho ESP32:  
&emsp;&emsp;`WiFi.mode(WIFI_STA)`: đặt ESP32 ở chế độ trạm (Station mode)  
&emsp;&emsp;`WiFi.macAddress()`: lấy địa chỉ MAC của ESP32 peer  

`#include <esp_now.h>`  
&emsp;Thư viện hỗ trợ giao tiếp không  của ESP32:  
&emsp;&emsp;`esp_now_init()`: khởi tạo giao thức ESP-NOW  
&emsp;&emsp;`esp_now_add_peer(&x)`: thêm thiết bị nhận vào danh sách  
&emsp;&emsp;`esp_now_send(mac, (uint8_t*)n, sizeof(n))`: cấu trúc thông tin của thiết bị nhận: địa chỉ mac thiết bị nhận, con trỏ đến dữ liệu cần, kích thước gói dữ liệu  
&emsp;&emsp;`esp_now_send(mac, (uint8_t*)n, sizeof(n))`: cấu trúc thông tin của thiết bị: địa chỉ mac thiết bị nhận, con trỏ đến dữ liệu cần, kích thước gói dữ liệu  
...
&emsp;Thư viện hỗ trợ giao tiếp không  của ESP32:  
&emsp;&emsp;`esp_now_init()`: khởi tạo giao thức ESP-NOW  
&emsp;&emsp;`esp_now_register_recv_cb()`: gọi hàm callback khi dữ liệu được gửi  
&emsp;&emsp;`esp_now_send(mac, (uint8_t*)n, sizeof(n))`: cấu trúc thông tin của thiết bị gửi  
...
Kết quả thực thi  
&emsp;Sau khi nạp code, Serial Monitor sẽ hiển thị:  
&emsp;&emsp;1. In ra địa chỉ IP của ESP32  
&emsp;&emsp;2. Sau khi nhận được dữ liệu từ thiết bị gửi, in ra `HELLO`  
Ví dụ:
```python
192.168.1.37
....................
